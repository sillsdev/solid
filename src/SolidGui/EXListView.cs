/* originally from http://www.codeproject.com/cs/miscctrl/EXListView.asp
 * 
 * This allows us to do more with sub items, like embed images.
 * 
 Hatton fixed up some things, but it's still pretty sustpect code.  The author
 apparently did not know about the need to dispose of graphics objects.
 */
...
//            }
        ...
        private void DrawLabel(DrawListViewSubItemEventArgs e, int x, int fonty)
        {
            if (e.Item.Selected) //hatton
            {
                e.Graphics.DrawString(e.SubItem.Text, e.SubItem.Font, SystemBrushes.HighlightText, x, fonty);
            }
            else
            {
                using( SolidBrush textBrush =   new SolidBrush(e.SubItem.ForeColor))
                {
                    e.Graphics.DrawString(e.SubItem.Text, e.SubItem.Font, textBrush, x, fonty);
                }           
            }
        }
        ...
        private void this_ColumnClick(object sender, ColumnClickEventArgs e) {
            ...
            if (_sortcol == 0) {
                ...
                if (this.Items[0].SubItems.Count > _sortcol)//hatton
                {
                    //ListViewSubItem
