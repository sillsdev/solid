...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="*[@lift='confer']">
    <xsl:if test="not(data = '')">
      <xsl:variable name="target"
        select="/root/entry[descendant::*[@lift=&quot;lexicalUnit&quot;]/data/text() = current()/data/text() or descendant::*[@lift=&quot;citationForm&quot;]/data/text() = current()/data/text()]"/>
  ...
  <xsl:template match="*[@lift='lexicalUnit']" name="lexicalUnit">
    ...
    <xsl:apply-templates
      select="descendant::*[@lift='lexicalRelationType'][not(ancestor::*[@lift='sense' or @lift='variant'])]"/>
  ...
  <xsl:template match="*[@lift='dateModified']">
    ...
    <xsl:variable name="month">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:message terminate="yes"> Unknown Month <xsl:value-of select="$monthAsAbbrev"/>
          </xsl:message>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
